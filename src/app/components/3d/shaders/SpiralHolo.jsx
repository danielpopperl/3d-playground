...
export default function SpiralHolo(camera) {
  ...
  const fragmentShader = `
      ...
      void main() {
        ...
        float mix_factor1 = sin(uv.x * PI + time * 0.7) * 0.5 + 0.5;
        float mix_factor2 = cos(uv.y * PI + time * 0.9) * 0.5 + 0.5;
        
        vec3 final_color = mix(color1, color2, 0.0);
        final_color = mix(final_color, color3, 0.0);
        
        float spiral = sin((atan(uv.x - 0.5, uv.y - 0.5) / PI * 2.0 + 25.5) * 10.0);
        final_color *= spiral;
